...
public class PlayerControl : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        {
            DontDestroyOnLoad(gameObject);  //Dont Destroy this object when you load a new scene. ONLY if instance is null. if you have not yet seen a player control. first one?
            instance = this; //set instance to this object. the CURRENT script that you are in 
        }
        else //if the instance is already set to an object. if there is a player control already this is the 2nd instance we've seen, so GET rid of it 
        {
            Destroy(gameObject);  //destroy this new object, so there is only ever one!! how ominous
        }
    }
